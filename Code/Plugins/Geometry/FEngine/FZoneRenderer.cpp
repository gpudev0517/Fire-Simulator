...
void FZoneRenderer::render(NESpatial* spatial, IGLSurface* surface)
{
    ...
    if (FZone* zone = qobject_cast<FZone*>(spatial))
    {
        if(zone->renderImage())
        {
            NENode* plane = NESCENE.createObject("Floor Plane", zone->zoneName());
            if(plane == NULL)
                plane = NESCENE.findChildNode(zone->zoneName());
            if (NEFloorPlane* floorPlane = qobject_cast<NEFloorPlane*> (plane))
            {
                floorPlane->isShow = true;
                floorPlane->m_textureImage = zone->textureImage();
                floorPlane->m_floorElevation = zone->FloorElevation();
                floorPlane->m_xsize = zone->XSize();
                floorPlane->m_zsize = zone->ZSize();
                floorPlane->m_swap = zone->Swap();
                floorPlane->isRevert = zone->Revert();
                floorPlane->m_xmin = zone-> XMin();
                floorPlane->m_zmin = zone->ZMin();
                floorPlane->m_xmax = zone->XMax();
                floorPlane->m_zmax = zone->ZMax();
