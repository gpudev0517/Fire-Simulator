#include "FDYNColorPresetDialog.h"
...
FDYNColorPresetDialog::FDYNColorPresetDialog(NENode* node, QWidget *parent) : QDialog(parent)
{
    m_node = node;
    ...
    connect(m_contentsWidget, &QListWidget::itemChanged, this, &FDYNColorPresetDialog::itemChanged);
    ...
    connect(addButton, &QAbstractButton::clicked, this, &FDYNColorPresetDialog::addPreset);
    connect(deleteButton, &QAbstractButton::clicked, this, &FDYNColorPresetDialog::deletePreset);
    connect(loadButton, &QAbstractButton::clicked, this, &FDYNColorPresetDialog::loadPreset);
    connect(applyButton, &QAbstractButton::clicked, this, &FDYNColorPresetDialog::applyPreset);

    connect(m_graphicScene, &QGraphicsScene::changed, this, &FDYNColorPresetDialog::graphicsViewChanged);
...
void FDYNColorPresetDialog::initPresetListWidget()
{
    createItem(m_node, m_currentWidget, Qt::AlignLeft, _LIST1_ICON_WIDTH_, _LIST1_ICON_HEIGHT_);
    m_currentNodeColorRamp = new NEColorRamp();
    m_currentNodeColorRamp->setColorRamp(m_node->getCurrentForPreset()->getColorRamp());

    m_node->LoadColorRampFromPreset(&m_colorPresetListMap);
...
void FDYNColorPresetDialog::createItem(NENode* node, QListWidget* listWidget, int alignment, int imageWidth, int imageHeight, QString itemStr)
{
    QListWidgetItem *colorPresetItem = new QListWidgetItem();
    colorPresetItem->setTextAlignment(alignment);
    if (node)
    {
        NEColorRamp* colorRamp = node->getCurrentForPreset();
    ...
    {
        if (node)
            colorPresetItem->setText(QString("Default %1").arg(node->objectName()));
        else
...
void FDYNColorPresetDialog::addPreset()
{
    ...
    if (oldItems.size() <= 0)
    {
        createItem(m_node, m_contentsWidget, Qt::AlignLeft /*AlignHCenter*/ | Qt::AlignVCenter, _LIST_ICON_WIDTH_, _LIST_ICON_HEIGHT_, itemStr);
        m_colorPresetListMap.insert(itemStr, m_node->getCurrentForPreset());
    }
}

void FDYNColorPresetDialog::deletePreset()
{
...
//            std::cout << "FDYNColorPresetDialog::deletePreset row:" << rowNumber << std::endl;
            delete m_contentsWidget->takeItem(rowNumber);
        }
    }
}

void FDYNColorPresetDialog::loadPreset()
{
    ...
    if (m_colorPresetListMap.contains(itemStr))
    {
        ...
        NEColorRamp* nodeColorRamp = m_node->getCurrentForPreset();
...
void FDYNColorPresetDialog::applyPreset()
{
    if (!m_node) return;
    QMap<QString, NEColorRamp*>::iterator iter;
    QString         itemStr;
    ...
    for(iter = m_colorPresetListMap.begin(); iter != m_colorPresetListMap.end(); iter++)
    {
        itemStr = iter.key();
        colorRamp = *iter;
//        std::cout << "FDYNColorPresetDialog::applyPreset " << itemStr.toStdString() << std::endl;
    }
    m_node->SaveColorRampToPreset(&m_colorPresetListMap);
...
void FDYNColorPresetDialog::updateCurColorRampAndValueTableWithoutName()
{
    ...
    createItem(m_node, m_currentWidget, Qt::AlignLeft, _LIST1_ICON_WIDTH_, _LIST1_ICON_HEIGHT_, itemStr);
    m_currentNodeColorRamp->setColorRamp(m_node->getCurrentForPreset()->getColorRamp());
    setValueToTable(m_currentNodeColorRamp);
}

void FDYNColorPresetDialog::itemChanged(QListWidgetItem *item)
{
    if (!item) return;
    ...
    QString         oldItemStr;
    NEColorRamp*    colorRamp;
//    std::cout << "FDYNColorPresetDialog::currentTextChanged current: " << item->text().toStdString() << std::endl;
    for(iter = m_colorPresetListMap.begin(); iter != m_colorPresetListMap.end(); iter++)
    {
        oldItemStr = iter.key();
        colorRamp = *iter;
...
//            std::cout << "FDYNColorPresetDialog::currentTextChanged old: " << oldItemStr.toStdString() << std::endl;
            break;
        }
    }
}

QIcon FDYNColorPresetDialog::crateColorRampPresetImage(NEColorRamp* colorRamp, int imageWidth, int imageHeight)
{
...
//        std::cout << "FDYNColorPresetDialog::crateColorRampPresetImage colorSize=" << colorSize << std::endl;
        QColor  oldColor, curColor;
        for (int i = 0; i < colorSize; i++)
        {
//            std::cout << "FDYNColorPresetDialog::crateColorRampPresetImage 1: i=" << i << ", r=" << colorList[i].second.red() << ", g=" << colorList[i].second.green() << ", b=" << colorList[i].second.blue() << std::endl;
...
void FDYNColorPresetDialog::createValueTable()
{
...
//            this, &FDYNColorPresetDialog::activeOfItem);
}

void FDYNColorPresetDialog::setValueToTable(NEColorRamp* colorRamp)
{
    QVector<QPair<qreal, QColor>>& colorList = colorRamp->getColorRampRef();
    int     colorSize = colorList.size();
    QColor  curColor;
    ...
    for (int i = 0; i < colorSize; i++)
    {
//        std::cout << "FDYNColorPresetDialog::setValueToTable i=" << i << ", r=" << colorList[i].second.red() << ", g=" << colorList[i].second.green() << ", b=" << colorList[i].second.blue() << std::endl;
//        if (colorList[i].first < 0) continue;
...
void FDYNColorPresetDialog::setValueToGraphicView(NEColorRamp* colorRamp)
{
    QVector<QPair<qreal, QColor>>& colorList = colorRamp->getColorRampRef();
    int     colorSize = colorList.size();
    QColor  curColor;
...
void FDYNColorPresetDialog::graphicsViewChanged(QList<QRectF> region){
    if(region.size() > 1){
...
void FDYNColorPresetDialog::getValueToTable(NEColorRamp* colorRamp)
{
    QVector<QPair<qreal, QColor>>& colorList = colorRamp->getColorRampRef();
    colorList.clear();
    int     colorSize = m_valueTable->rowCount();
