        ...
        */
        ...
        if(NEObject* neObject = qobject_cast<NEObject*>(const_cast<QObject*>(child)))
        {
            ...
            if(neObject->isHidden())
            {
                ...
                {
                    //qDebug() << "Node with non-initialized factory when traversing scene-inspector tree.";
                    continue;
                }
                ...
                //if(node->factory().nodeInherits() != "Groups")
                {
                    ...
                    //qDebug() << keyString;
                    FDYNSceneInspectorItem *pp = qobject_cast< FDYNSceneInspectorItem*>( (parent->parent()) );
                    while (pp) {
                        keyString += "," + pp->text(0);
                        //qDebug() << keyString;
                    ...
                    if(m_searchText.isEmpty() || containStringinChildren(tmpItem->Object(), m_searchText)){
                        ...
                        if(!m_searchText.isEmpty() && tmpItem->Object()->objectName().contains(m_searchText, Qt::CaseInsensitive)){
