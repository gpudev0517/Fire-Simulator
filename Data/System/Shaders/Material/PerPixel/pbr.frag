...
vec4 ShadingModel(vec3 N, vec3 P, vec3 L, vec4 lightColor, float intensity, vec4 vertexColor, vec3 materialValue)
{
	...
	vec3 specular     = nominator / denominator * specularIntensity;

	// add to outgoing radiance Lo
	float NdotL = max(dot(N, L), 0.0);
	// reflectance equation
	vec3 Lo = (kD * albedo / PI + specular) * lightColor.xyz * NdotL * intensity;

    vec3 color = Lo;

	return vec4(color, vertexColor.a * lightColor.a);
}
