...
public:
    ...
    QString  GetUnknownItemsLocName();
    void     SetUnknownItemsLocName( QString name );

    bool SetUIProjectSetting(QString name, QString val);
    QString GetUIProjectSetting(QString name);

    bool SetUIAppSetting(QString name, QString val);
    QString GetUIAppSetting(QString name);
...
uint FEngine::init(uint initMode)
{
    ...
    P->m_DBFile.setFilter( "All (*.*);;SQ Lite(*.sqlite);;FRI3D(*.f3d);" );
    ...
    if(NESpatial* ln = qobject_cast<NESpatial*>(lightnode))
    {
        ...
        //ln->SetOrientation( eulerf( 20, -10, 160 ) );
    }
    ...
    QString loadUIStr = P->GetUIAppSetting( "LoadUI" );
    if( loadUIStr == "true" )
    {
        emit sendLoadUIOption( true );
    }
    else
    {
        emit sendLoadUIOption( false );
    }

    QString saveUIStr = P->GetUIAppSetting( "SaveUI" );
    if( saveUIStr == "true" )
    {
        emit sendSaveUIOption( true );
    }
    else
    {
        emit sendSaveUIOption( false );
    }
...
void FEngine::simulateScenario(QString scnName, QString desc, QString simEngineNodeName)
{
    ...
    if( cfastengine )
    {
        ...
        //
        // Save the model before running CFAST
        //
        P->saveModelItems("");
...
void FEngine::setActiveZone( QString zoneName )
{
    ...
    //qDebug() << "Active ui data " << P->m_UIData;
    emit sendLoadUIData( P->m_UIData );
...
uint FEnginePrivate::loadFRANXFile(const QString& filePath )
{
    ...
    if( dbConvertProgramPath == "" )
    {
        QString path = QCoreApplication::applicationDirPath();
        QStringList paths;
        paths.append( path );
        dbConvertProgramPath =  QStandardPaths::findExecutable( "ImportFRANX",  paths );
    }
...
uint FEnginePrivate::loadPDMSFile(const QString& filePath )
{
    ...
    if( dbConvertProgramPath == "" )
    {
        ...
        QStringList paths;
        paths.append( path );
        dbConvertProgramPath =  QStandardPaths::findExecutable( "ImportPDMS",  paths );
    }
...
//        qDebug() << v;
        ...
        if( p.userType() == (int)getNEVarIds().filePathId  )
        {
            propValObj[ "type" ]  = p.userType();
            FilePath sv = v.value<FilePath>();
            propValObj[ "value" ] = sv.path();
        }
...
uint FEnginePrivate::loadDBFile(const QString& filePath)
{
    ...
    QString mloadUIStr = GetUIAppSetting( "LoadUI" );
    m_UIData.clear();
    if( mloadUIStr == "true" )
    {
        m_UIData = S->loadUIData();

        QFile sceneFilefri3d( scenefile );
        if( sceneFilefri3d.exists( ) )
        {
            sceneFilefri3d.open( QFile::ReadOnly );
            m_UIData = sceneFilefri3d.readAll();
        }

        //emit S->sendLoadUIData( scenefile );
        //emit S->sendLoadUIData( uidata );
    }
...
void FEngine::save(QString filename)
{
    ...
    if( saveUIStr == "true" && uidir.exists() )
    {
        NEBASE.SetSaveDir( scenefolder );
        NEBASE.SetCacheDir( scenefolder + "/" + dbfileinfo.baseName() );
...
void FEngine::updateLoadUIOption(bool loadUI)
{
    P->m_loadUI = loadUI;

    QString loadUIStr = "true";
    if( loadUI == true )
        loadUIStr = "true";
    else
        loadUIStr = "false";
    P->SetUIAppSetting( "LoadUI", loadUIStr );
}


void FEngine::updateSaveUIOption(bool saveUI)
{
    QString saveUIStr = "true";
    P->m_saveUI = saveUI;
    if( saveUI == true )
        saveUIStr = "true";
    else
        saveUIStr = "false";
    P->SetUIAppSetting( "SaveUI", saveUIStr );
}


QString FEngine::loadUIData()
{
    QString key = "FRI3DUI";
    QString val = P->GetUIProjectSetting( key );
    //qDebug()<< "UI Val " << val;
    return val;
}

void FEngine::saveUIData(QString jsonStr)
{
    QString key = "FRI3DUI";
    P->SetUIProjectSetting( key, jsonStr );
    //qDebug() << "UIII " << jsonStr;
    return;
}
...
QString FEnginePrivate::GetUnknownItemsLocName()
{
    f_GetUnknownItemsLocName gco =
            (f_GetUnknownItemsLocName) m_pLibrary.resolve( "GetUnknownItemsLocName" );
    if (!gco)
    {
        std::cerr << "could not locate the function GetUnknownItemsLocName" << std::endl;
        return QString("");
    }
...
void FEnginePrivate::SetUnknownItemsLocName(QString name)
{
    f_SetUnknownItemsLocName acl =
            (f_SetUnknownItemsLocName) m_pLibrary.resolve( "SetUnknownItemsLocName" );
    if (!acl)
    {
        std::cerr << "could not locate the function SetUnknownItemsLocName" << std::endl;
        return ;
    }
...
bool FEnginePrivate::SetUIProjectSetting(QString name, QString val)
{
    f_SetUIProjectSetting suips =
            (f_SetUIProjectSetting) m_pLibrary.resolve( "SetUIProjectSetting" );
    if (!suips)
    {
        std::cerr << "could not locate the function SetUIProjectSetting" << std::endl;
        return false;
    }
    std::string s = name.toStdString();
    std::string s1 = val.toStdString();


    bool ret = suips(&s[0], &s1[0] ) ;
    return ret;
}


QString FEnginePrivate::GetUIProjectSetting(QString name)
{
    f_GetUIProjectSetting guips =
            (f_GetUIProjectSetting) m_pLibrary.resolve( "GetUIProjectSetting" );
    if (!guips)
    {
        std::cerr << "could not locate the function GetUIProjectSetting" << std::endl;
        return 0;
    }
...
bool FEnginePrivate::SetUIAppSetting(QString name, QString val)
{
    f_SetUIAppSetting suiaps =
            (f_SetUIAppSetting) m_pLibrary.resolve( "SetUIAppSetting" );
    if (!suiaps)
    {
        std::cerr << "could not locate the function SetUIProjectSetting" << std::endl;
        return false;
    ...
    bool ret = suiaps(&s[0], &s1[0] ) ;
    return ret;
}


QString FEnginePrivate::GetUIAppSetting(QString name)
{
    f_GetUIAppSetting guiaps =
            (f_GetUIAppSetting) m_pLibrary.resolve( "GetUIAppSetting" );
    if (!guiaps)
    {
        std::cerr << "could not locate the function GetUIAppSetting" << std::endl;
        return 0;
    }
