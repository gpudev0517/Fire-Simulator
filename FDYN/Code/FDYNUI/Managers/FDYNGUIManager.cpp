...
#include "Windows/ExternalRendererSettings/FDYNExternalRendererSettingsWindow.h"
...
void FDYNGUIManagerPrivate::init()
{
    ...
    m_pExtRenderWindow = new FDYNExternalRendererSettingsWindow(m_pMainWindow);
...
void FDYNGUIManagerPrivate::postInit()
{
    ...
    NENode* cfnode = qobject_cast< NENode *>( NESCENE.createObject("CFAST", "CFAST", en) );
    if( cfnode )
    {
...
{
    QSettings settings( "Fire Dynamics", "FRI3D" );
    FDYNFileDialog dlg(mainWindow(), settings.value("LastSaveDB").toByteArray(),
    ...
    {
        fileNames = dlg.selectedFiles();
        settings.setValue("LastSaveDB", dlg.directory());
    }
...
void FDYNGUIManager::loadUI(QString filename)
{
    ...
    {
        sceneFilefri3d.open( QFile::ReadOnly );
        fromJson( sceneFilefri3d.readAll() );
    }
    else
    {

        //
        // Clear all UI Objects and Visual Objects associated with the zone
        //
        const QList<NEMeasurementItem*> mitems = NESCENE.findChildren<NEMeasurementItem*>();
...
void FDYNGUIManager::fromJson( QString jsonStr )
{
    QJsonDocument jsonDoc = QJsonDocument().fromJson( jsonStr.toUtf8() );
    QJsonArray jsonArray = jsonDoc.array();
    P->fromJson( this, jsonArray );
}


QString FDYNGUIManager::toJson()
{
    QJsonArray guiitems;
    P->toJson( this, guiitems );
    P->toJson( &NESCENE, guiitems );

    QJsonObject allobjects = QJsonDocument(guiitems).object();
    QJsonDocument saveDoc(  guiitems );
    //qDebug() << saveDoc;
    QString retJson = saveDoc.toJson();
    return retJson;
}
...
void FDYNGUIManager::saveUI(QString filename)
{
    if( filename != "" )
    {
        ...
        {
            QString jsonStr = toJson();
            sceneFilefri3d.write( jsonStr.toUtf8() );
        }
    }
    else
    {
        QString jsonStr = toJson();
        emit sendSaveUIData( jsonStr );
    }
...
{
    QSettings settings("Fire Dynamics", "FRI3D");
    FDYNFileDialog dlg(mainWindow(), settings.value("LastImportDrawing").toString(),
    ...
    if( dlg.exec() == QDialog::Accepted )
    {
        ...
        settings.setValue("LastImportDrawing", dlg.directory());
    }
...
{
    QSettings settings("Fire Dynamics", "FRI3D");
    FDYNFileDialog dlg(mainWindow(), settings.value("LastImportFRANX").toString(),
    ...
    {
        fileName = dlg.selectedFile();
        settings.setValue("LastImportFRANX", dlg.directory());
    }
...
void FDYNGUIManager::editPreferences()
{
    ...
    if( dlg->exec() == QDialog::Accepted)
    {
        ...
        emit sendLoadUIOption( dlg->loadUIData() );
        emit sendSaveUIOption( dlg->saveUIData() );
    }
}
...
void FDYNGUIManager::updateSceneInspector()
{
    ...
    qDebug() << "mainWindow()->sceneInspectorWindow()->update():" << std::chrono::duration_cast<std::chrono::milliseconds>(endTime-startTime).count() << "msec.";
...
void FDYNGUIManager::objectCreated( NENode *object )
{
    ...
    if( NESpatialEngine* spatialEngine = qobject_cast< NESpatialEngine *>( object ))
    {
        ...
        connect( this, SIGNAL(sendLoadUIOption(bool)), spatialEngine, SLOT(updateLoadUIOption(bool) ) );
        connect( spatialEngine, SIGNAL(sendLoadUIOption(bool)), this, SLOT(updateLoadUIOption(bool) ) );

        connect( this, SIGNAL(sendSaveUIOption(bool)), spatialEngine, SLOT(updateSaveUIOption(bool) ) );
        connect( spatialEngine, SIGNAL(sendSaveUIOption(bool)), this, SLOT(updateSaveUIOption(bool) ) );

        connect( this, SIGNAL(sendLoadUIData()), spatialEngine, SLOT(loadUIData()) );
        connect( this, SIGNAL(sendSaveUIData(QString)), spatialEngine, SLOT(saveUIData(QString) ) );
...
void FDYNGUIManager::updateLoadUIOption(bool val)
{
    FDYNSettingsDialog* dlg = mainWindow()->settingsDialog();
    if( !dlg )
        return;

    dlg->setLoadUIData( val );

    return;
}

void FDYNGUIManager::updateSaveUIOption(bool val)
{
    FDYNSettingsDialog* dlg = mainWindow()->settingsDialog();
    if( !dlg )
        return;
    dlg->setSaveUIData( val );
    return;
}
...
void FDYNGUIManager::updateOfflineRenderer()
{
...
void FDYNGUIManager::finishedOfflineProcess()
{
    P->m_locker.lock();
...
void FDYNGUIManager::startedOfflineProcess()
{
    P->m_locker.lock();
...
void FDYNGUIManager::updateOfflineProgress(float val)
{
    P->m_locker.lock();
...
void FDYNGUIManager::finishedOfflineRendering()
{
    foreach (NEGLOfflineRenderer *renderer, NERENDERMAN.m_offlineRenderers) {
        delete renderer;
    }
...
void FDYNGUIManager::startedOfflineRendering()
{
    P->timeline()->setOfflineRecordingMode(true);
}
...
bool FDYNGUIManager::event(QEvent *e)
{
...
void FDYNGUIManager::setMeasure(bool bMeasure)
{
    ...
    QMetaObject::invokeMethod(cadToolObj, "setBtnMeasure",
                              Q_RETURN_ARG(QVariant, returnValue),
                              Q_ARG(QVariant, bMeasure));
...
void FDYNGUIManager::setMeasureEdge(bool bMeasureEdge)
{
    ...
    QMetaObject::invokeMethod(cadToolObj, "setBtnMeasureEdge",
                              Q_RETURN_ARG(QVariant, returnValue),
                              Q_ARG(QVariant, bMeasureEdge));
    updateGLWindows();
}
...
void FDYNGUIManager::showOfflineRendererSettingsDialog()
{
...
void FDYNGUIManagerPrivate::toJson(QObject *parent, QJsonArray &guiitems)
{
    QJsonObject jsonObject;
    //qDebug() << parent->objectName();

    if(NENode* neNode = qobject_cast<NENode*>(parent))
    {
        if( neNode->factory().nodeInherits() != "Groups" &&
            neNode->factory().nodeInherits() != "Forces" &&
            neNode->factory().nodeInherits() != "Physical" &&
            neNode->factory().nodeInherits() != "Geometry" &&
            neNode->factory().nodeInherits() != "Cameras"
           )
        {
            //QJsonArray propList = neChild->getJSONFromProperties( );
            QJsonArray propList = getJSONFromProperties( qobject_cast< QObject *>( neNode ) );

            jsonObject["factoryNodeName"] = neNode->factory().nodeName();
            jsonObject["factoryNodeType"] = (int) neNode->factory().nodeType();
            jsonObject["factoryNodeInherits"] = neNode->factory().nodeInherits();
            jsonObject["factoryVersion"] = neNode->factory().nodeVersion();
            jsonObject["factoryNodeObjectName"] = neNode->factory().objectName();
            jsonObject["name"] = neNode->objectName();
            if( neNode->parent() )
                jsonObject["parent"] = neNode->parent()->objectName();
            jsonObject["properties" ] = propList;

            //
            // If measurement item save and restore needed properties.
            //
            NEMeasurementItem* mitem = qobject_cast< NEMeasurementItem *>( neNode );
            if( mitem )
            {
                QString str = mitem->toJson();
                QJsonDocument jdoc = QJsonDocument::fromJson( str.toUtf8() );


                jsonObject[ "measurementInfo" ] = jdoc.object();
            }

            //qDebug() << "json " << neNode->factory().nodeName();
            guiitems.append( jsonObject );
        }
    }
    else if( const FDYNGUIManager* fgui = qobject_cast<const FDYNGUIManager*>(parent))
    {
        QJsonObject propObj;
        jsonObject["name"] = parent->objectName();
        jsonObject["properties" ] = getJSONFromProperties( parent );
        guiitems.append( jsonObject );
        //qDebug() << jsonObject ;
    }
    QObjectList objChildren = parent->children(); // Get a reference to the list of all children of the scene
    for(QObject* child : objChildren)
    {
        QJsonObject   jsonObject ;
        toJson( child, guiitems );
    }
}


void FDYNGUIManagerPrivate::fromJson( QObject *node, QJsonArray &jsonArray )
{
    for( int i = 0; i < jsonArray.size(); ++i )
    {
        ...
        QString nname       = jsonObject["name"].toString();
        QJsonArray props;

        if( jsonObject.contains( "factoryNodeName" ) == false )
        {
            //qDebug() << "Node " << node->objectName();
            if( jsonObject.contains( "properties") == false )
            {
                //qDebug() << "Setting Property for Node " << node->objectName();
                setPropertyFromJson( node, jsonObject );
                NEFloorPlane* ssnode = qobject_cast< NEFloorPlane *>( node );
                if( ssnode )
                {
                    ssnode->SetShowFloorPlan( false );
                }
            }
            else
            {
                props    = jsonObject[ "properties"].toArray();
                QObject* snode = node->findChild<QObject*>( nname, Qt::FindChildrenRecursively  );
                if( snode )
                {
                    //qDebug() << "Setting Properties for existing node " << snode->objectName() ;
                    fromJson( snode, props );
                }
                else
                {
                    fromJson( node, props );
        ...
        {
            NENode* snode =  NESCENE.findChildNode( nname );
            //qDebug() << "Node Names " << nname;

            if( !snode )
            {
                QString factoryNodeName = jsonObject["factoryNodeName"].toString() ;
                QString parentNodeName  = jsonObject["parent"].toString() ;

                NENode* pnode =  NESCENE.findChildNode( parentNodeName );

                if( pnode )
                    snode = qobject_cast< NENode *> ( NESCENE.createObject( factoryNodeName, nname, pnode ) );
                else
                    snode = qobject_cast< NENode *> ( NESCENE.createObject( factoryNodeName, nname ) );
                //qDebug() << "Created " << snode->objectName();
            ...
            //
            // If measurement item restore
            //
            NEMeasurementItem* mitem = qobject_cast< NEMeasurementItem *>( snode );
            if( mitem )
            {
                //qDebug() << "Measurement item " << mitem->objectName();
                QJsonObject jsonobj = jsonObject[ "measurementInfo" ].toObject();
                QJsonDocument jdoc( jsonobj );
                QString str = jdoc.toJson();
                //qDebug() << "Json for MITEM " << jdoc;
                mitem->fromJson( str  );
            }
        }
    }
...
QJsonArray FDYNGUIManagerPrivate::getJSONFromProperties( const QObject *obj )
{
    ...
    {
        QJsonObject childjobj;
        childjobj[ "name" ] = child->objectName();
        childjobj[ "properties" ] = getJSONFromProperties( child ) ;
        retval.append( childjobj );
    }
...
void FDYNGUIManagerPrivate::setPropertyFromJson(QObject *node, QJsonObject &jsonObject)
{
    ...
    {
        QString vpath = v.value<QString>();
        FilePath vvpath;
        vvpath.setPath( vpath );
        if( !vvpath.path().isEmpty())
        {
            if (pv.isWritable()) node->setProperty( propertyName.toUtf8(), vvpath );
        }
    }
