...
void FDYNFRANXInspector::updateChildren(const QObject* child)
{
    ...
    //
    createRootOrganizerItems( m_pRoot, childItem );
    //
...
FDYNFRANXInspectorItem*  FDYNFRANXInspector::createRootOrganizerItems( QObject *rootscn, FDYNFRANXInspectorItem* rootItem )
{
...
FDYNFRANXInspectorItem* FDYNFRANXInspector::createOrganizerItem( QString categoryName, FDYNFRANXInspectorItem *objItem )
{
    QRegExp rx("\\/");
    QStringList query = categoryName.split(rx);

    FDYNFRANXInspectorItem* objOrgItem = objItem;
    FDYNFRANXInspectorItem* parentOrgItem = objOrgItem;
    ...
    FDYNFRANXInspectorItem *pp = dynamic_cast< FDYNFRANXInspectorItem *>( objItem );
    ...
    QString ks = query[0];
    FDYNFRANXInspectorItem* item = nullptr;
    ...
    for( auto i = 0 ; i < query.size(); ++i )
    {
        ...
        item = new FDYNFRANXInspectorItem();
        ...
        if(NEObject* neObject = qobject_cast<NEObject*>(const_cast<QObject*>(child)))
        {
            ...
            //if(neObject->isHidden())
            {
                ...
                //if(node->factory().nodeInherits() != "Groups")
                {
                    ...
                    {
                        QString keyString = zoneItem->otherCategoryType();
                        FDYNFRANXInspectorItem *pp = dynamic_cast<FDYNFRANXInspectorItem *>( parent );
                        while( pp )
                        {
                            keyString += "," + pp->text(0);
                            pp = dynamic_cast<FDYNFRANXInspectorItem *>(pp->parent());
