...
public:
    ...
    QAction *offlineRenderSettingAct;
    QAction *offlineRenderSeqAct;
    QAction *offlineRenderCurrentAct;
    QAction *offlineCancelRenderAct;
    ...
    : QMainWindow(parent, flags)
{
    ...
    const QIcon offrenderSettingIcon = QIcon(":/img/16/offline_render_setting.png");
    const QIcon offrenderOneIcon = QIcon(":/img/16/offline_render_one.png");
    const QIcon offrenderSeqIcon = QIcon(":/img/16/offline_render_sequence.png");

    P->offlineRenderSettingAct = new QAction(offrenderSettingIcon, "&Offline Render Setting");
    P->offlineRenderCurrentAct = new QAction(offrenderOneIcon, "&Offline Render Current");
    P->offlineRenderSeqAct = new QAction(offrenderSeqIcon, "&Offline Render Sequence");
    P->offlineCancelRenderAct = new QAction("&Cancel Render");
...
//    P->menuGizmo = P->menuVisual->addMenu("&GIZMOS");
    ...
    renderMenu->addAction(P->offlineRenderSettingAct);
    renderMenu->addAction(P->offlineRenderCurrentAct);
    renderMenu->addAction(P->offlineRenderSeqAct);
    renderMenu->addAction(P->offlineCancelRenderAct);
...
void FDYNMainWindow::open( QString filePath )
{
    ...
    FDYNFileDialog dlg(this, settings.value("LastLoadDB").toByteArray(),
    ...
    else
    {
        if( dlg.exec() == QDialog::Accepted )
        {
            ...
            settings.setValue("LastLoadDB", dlg.directory());
            settings.sync();
            FDYNGUI.loadDB(fileNames[0]);
            if (!fileNames[0].isEmpty())
                P->setCurrentFile(fileNames[0]);
...
void FDYNMainWindow::renderCurrentFrame()
{
    ...
    for(GLDockWidgetBundle *w : FDYNGUI.P->m_IDtoGLDockWidget){
...
void FDYNMainWindow::renderSequence()
{
    ...
    QString startFrame = QString::number(FDYNGUI.P->timeline()->playbackStartFrame());
    QString endFrame = QString::number(FDYNGUI.P->timeline()->playbackEndFrame());
...
void FDYNMainWindow::removeSelected3DItem()
{
    if( P->zoneMainWindow )
        P->zoneMainWindow->removeSelected3D();
    //
    // If its a visual item just remove it
    //
    NENode* selNode = NESCENE.selectedNode();
    if( P->sceneMainWindow && selNode->factory().nodeName() == "Measurement Item" )
        P->sceneMainWindow->removeSelectedItem();
}
...
void FDYNMainWindowPrivate::tweakUI()
{
    ...
    S->connect(offlineRenderSettingAct, &QAction::triggered, &FDYNGUI, &FDYNGUIManager::showOfflineRendererSettingsDialog);
    S->connect(offlineRenderCurrentAct, &QAction::triggered, S, &FDYNMainWindow::renderCurrentFrame);
    S->connect(offlineRenderSeqAct, &QAction::triggered, S, &FDYNMainWindow::renderSequence);
    S->connect(offlineCancelRenderAct, &QAction::triggered, &FDYNGUI, &FDYNGUIManager::finishedOfflineRendering);
    ...
    S->connect(removeAct, &QAction::triggered, S, &FDYNMainWindow::removeSelected3DItem);
