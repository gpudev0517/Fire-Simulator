...
public slots:
    ...
    void showVerticalHeaderContextMenu(QPoint pos);
    void deleteRows();

    void addT2();
    void drawGraph(int type);

    void changeMaterialInfoByGraph(int dataIndex, double x, double y, bool bChangeX);
...
private:
    ...
    QMenu *verticalHeaderContextMenu;

public:
    FireMaterial *m_CurMaterial;
    int m_CurMaterialColumeIndex = 0;
    QList<FireMaterial> m_MaterialList;
    bool m_bRescaleGraph = true;
};
