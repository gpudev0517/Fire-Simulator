    ...
    ui(new Ui::FDYNFireMaterialDialog)
{
    ...
    ui->tableFireMaterials->verticalHeader()->setContextMenuPolicy(Qt::CustomContextMenu);
    QObject::connect(ui->tableFireMaterials->verticalHeader(), SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(showVerticalHeaderContextMenu(QPoint)));

    verticalHeaderContextMenu = new QMenu(this);
    QAction *deleteAction = new QAction("Delete Row(s)", this);
    QObject::connect(deleteAction, SIGNAL(triggered()), this, SLOT(deleteRows()));
    verticalHeaderContextMenu->addAction( deleteAction );
...
void FDYNFireMaterialDialog::deleteRows()
{
    QList<QTableWidgetItem *> items = ui->tableFireMaterials->selectedItems();
    QList<int> rows;
    rows.clear();
    foreach(QTableWidgetItem *item, items){
        if( rows.indexOf(item->row()) == -1 ){
            rows.push_back(item->row());
        }
    }

    qSort(rows.begin(), rows.end(), variantLessThan);
    foreach(int row, rows){
        ui->tableFireMaterials->model()->removeRow(row);

        m_CurMaterial->m_Infos.removeAt(row);
...
void FDYNFireMaterialDialog::showVerticalHeaderContextMenu(QPoint pos)
{
    int column = ui->tableFireMaterials->verticalHeader()->logicalIndexAt(pos);

    verticalHeaderContextMenu->popup(ui->tableFireMaterials->horizontalHeader()->viewport()->mapToGlobal(pos));
}
...
void FDYNFireMaterialDialog::updateCurrentMaterial()
{
    ui->tableFireMaterials->clearContents();
    for( int j = 0; j < ui->tableFireMaterials->columnCount(); j++)
    {
...
void FDYNFireMaterialDialog::changeMaterialInfoByGraph(int dataIndex,  double x, double y, bool bChangeX)
{

    if( bChangeX){
        QTableWidgetItem *xitem = ui->tableFireMaterials->item(dataIndex, 0);
        if (dataIndex == 0 ){
            if (x >= 0){
                ui->tableFireMaterials->blockSignals(true);
                ui->tableFireMaterials->clearSelection();
                xitem->setSelected(true);
                ui->tableFireMaterials->scrollTo(ui->tableFireMaterials->model()->index(dataIndex, 0));
                ui->tableFireMaterials->blockSignals(false);

                xitem->setText(QString::number(x));
            }
        }
        else if ( dataIndex >= m_CurMaterial->m_Infos.size() - 1){
            QTableWidgetItem *xprevitem = ui->tableFireMaterials->item(dataIndex-1, 0);
            float prevx = xprevitem->text().toFloat();
            if( x > prevx){
                ui->tableFireMaterials->blockSignals(true);
                ui->tableFireMaterials->clearSelection();
                xitem->setSelected(true);
            ...
            QTableWidgetItem *xnextitem = ui->tableFireMaterials->item(dataIndex+1, 0);
            float nextx = xnextitem->text().toFloat();
        ...
        int yindex = m_CurMaterialColumeIndex == 0 ? 1 : m_CurMaterialColumeIndex;
        //int yindex = ui->tableFireMaterials->selectedItems().size() <= 0 ? 1 : ui->tableFireMaterials->selectedItems().last()->column();
        QTableWidgetItem *yitem = ui->tableFireMaterials->item(dataIndex, yindex);

        ui->tableFireMaterials->blockSignals(true);
        ui->tableFireMaterials->clearSelection();
        yitem->setSelected(true);
        ui->tableFireMaterials->scrollTo(ui->tableFireMaterials->model()->index(dataIndex, yindex));
        ui->tableFireMaterials->blockSignals(false);

        yitem->setText(QString::number(y));
    }
}

void FDYNFireMaterialDialog::showEvent(QShowEvent *e)
{
    QDialog::showEvent(e);

    ui->widgetGraph->m_Plot->clearGraphs();
    ui->widgetGraph->setDarkTheme();
    ui->widgetGraph->m_Plot->addGraph();
    ui->widgetGraph->m_Plot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectAxes | QCP::iSelectPlottables);
    ui->widgetGraph->m_Plot->graph()->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ScatterShape::ssCrossCircle));
    ui->widgetGraph->m_Plot->graph()->setSelectable(QCP::stSingleData);
    ui->widgetGraph->m_Plot->graph()->setPen(QPen(Qt::white));
...
void FDYNFireMaterialDialog::drawGraph(int index)
{
    ...
    if(m_CurMaterial){
        m_CurMaterialColumeIndex = index;
        ...
        if( m_CurMaterial->m_Infos.size() <= 0)
            ui->widgetGraph->drawPlot(x, y, xupper, yupper, "", m_bRescaleGraph);
        else{
            ...
            ui->widgetGraph->drawPlot(x, y, xupper, yupper, title, m_bRescaleGraph);
        }
    }
}
...
bool materialInfoLessThan(const FireMaterialInfo &v1, const FireMaterialInfo &v2)
{
    return v1.m_Time < v2.m_Time;
}

void FDYNFireMaterialDialog::materialInfoChanged(int row, int column)
{
    QList<QTableWidgetItem *> items = ui->tableFireMaterials->selectedItems();
    QTableWidgetItem *orgItem = ui->tableFireMaterials->item(row, column);
    if( items.size() > 0 ){
        ui->tableFireMaterials->blockSignals(true);
    }
    foreach(QTableWidgetItem *everyitem, items){
        //QTableWidgetItem *item = ui->tableFireMaterials->item(row, 0);
        QTableWidgetItem *item = everyitem;
        row = item->row();
        column = item->column();
        if(item && !item->text().isEmpty()){
            ...
            if(column == 0){
                item = ui->tableFireMaterials->item(row, 0);
                if(item)
                    info.m_Time = orgItem->text().toDouble();
                item->setText(QString::number(info.m_Time));
            }
            else if(column == 1){
                item = ui->tableFireMaterials->item(row, 1);
                if(item)
                    info.m_HRR = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_Height = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_Area = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_CO = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_Soot = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_HCN = orgItem->text().toDouble();
                ...
                if(item)
                    info.m_TS = orgItem->text().toDouble();

                item->setText(QString::number(info.m_TS));
            }

            m_CurMaterial->m_Infos.replace(row, info);
            qSort(m_CurMaterial->m_Infos.begin(), m_CurMaterial->m_Infos.end(), materialInfoLessThan);
