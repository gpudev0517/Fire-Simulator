...
{
public:
    FDYNTimelineWidgetPrivate(FDYNTimelineWidget* s) :
        S(s),
        m_currentFrame(0),
        ...
        m_TimelineUnits(FDYNTimelineWidget::Frames)
    ...
    FDYNTimelineWidget::NETimelineUnits m_TimelineUnits = FDYNTimelineWidget::Frames;
    QPainter* painter;
private:
    FDYNTimelineWidget* S;
};

FDYNTimelineWidget::FDYNTimelineWidget(QWidget* parent) :
    QWidget(parent),
    P(new FDYNTimelineWidgetPrivate(this))
{
    P->m_pTimeline = qobject_cast<NETimeline*>(parent);
}

FDYNTimelineWidget::~FDYNTimelineWidget()
{
    delete P->painter;
    delete P;
}

void FDYNTimelineWidget::changeUnits(FDYNTimelineWidget::NETimelineUnits val)
{
    P->m_TimelineUnits = val;
    repaint();
}

bool FDYNTimelineWidget::checkFramesTimeline()
{
    return P->m_TimelineUnits == FDYNTimelineWidget::Frames;
}

#define MIN_DIFF_TIME 0.01
#define MIN_DIFF_FRAME 1

void FDYNTimelineWidget::paintEvent(QPaintEvent* event)
{
    event->accept();
...
/*!
*/
double FDYNTimelineWidget::playbackStartFrame() const
{
    if(P->m_pTimeline)
...
/*!
*/
double FDYNTimelineWidget::playbackEndFrame() const
{
    if(P->m_pTimeline)
...
void FDYNTimelineWidget::setPlaybackStartFrame(double frame)
{
    //m_PlaybackStartFrame = frame;
    repaint();
}

void FDYNTimelineWidget::setPlaybackEndFrame(double frame)
{
    //m_PlaybackEndFrame = frame;
    repaint();
}

void FDYNTimelineWidget::setCurrentFrame(double frame)
{
...
void FDYNTimelineWidget::setCurrentFrame(const QString& frame)
{
    if (checkFramesTimeline())
...
void FDYNTimelineWidget::setKeyframeMarks(QList<uint> marks)
{
    P->m_keyframeMarks = marks;
    repaint();
}

void FDYNTimelineWidget::mousePressEvent(QMouseEvent* event)
{
...
void FDYNTimelineWidget::mouseReleaseEvent(QMouseEvent* event)
{
    if(event->button() != Qt::LeftButton)
        return;
...
void FDYNTimelineWidget::mouseMoveEvent(QMouseEvent* event)
{
    if(!P->m_mousePressed)
        return;
...
*/
bool FDYNTimelineWidget::computeBarPositions(const NEFrameInfo& frameInfo, float& barStartPos, float& barEndPos)
{
    int endFrame = playbackEndFrame();
    int startFrame = playbackStartFrame();
