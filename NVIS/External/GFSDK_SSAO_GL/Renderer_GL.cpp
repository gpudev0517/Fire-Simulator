...
int GFSDK::SSAO::GL::Renderer::AddRef()
{
    m_nRef++;
    return m_nRef;
}
...
#if 0 // for debug
        printf("m_InputDepth.Texture.GLTexture.TextureId : %d\n", m_InputDepth.Texture.GLTexture.TextureId);
        m_GL.glBindFramebuffer(GL_FRAMEBUFFER, 0);
        GLSLPrograms::Default_PS& defProgram = GetDefaultPS();
        defProgram.Enable(m_GL);
        //defProgram.SetTexture(m_GL, m_InputDepth.Texture.GLTexture);
        defProgram.SetTexture(m_GL, GFSDK_SSAO_Texture_GL(GL_TEXTURE_2D, m_FullResViewDepthTextureId));
        m_GL.glDrawArrays(GL_TRIANGLES, 0, 3);
#endif
    }
}
